...
then
echo '\n[X] sed package not found, please install'
exit -1
else
echo '\n[*] sed package found, proceeding'
fi
...
TOKENRESULT=$(echo $GETTOKEN | sed 's/\(.*\"result\"\:\ \"\)\([a-zA-Z0-9]*\)\(\".*\)/\2/')
TOKENID=$(echo $GETTOKEN | sed 's/\(.*\"canarytoken\"\:\ \"\)\([a-zA-Z0-9]*\)\(\".*\)/\2/')
